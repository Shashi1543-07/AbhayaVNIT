...
import { Plus, FileText, MapPin, Loader2, Activity, Trash2 } from 'lucide-react';
...
export default function Reports() {
    ...
    useEffect(() => {
        if (!user) {
            console.log('Reports: No user profile found, skipping subscription');
            setReports([]);
            setLoading(false);
            return;
        }

        console.log('Reports: Initializing subscription for user:', user.uid);
        const unsubscribe = incidentService.subscribeToUserIncidents(user.uid, (fetchedReports) => {
            console.log('Reports: Success! Fetched count:', fetchedReports.length);
            setReports(fetchedReports);
            setLoading(false);
        });

        // Safety timeout to prevent infinite spinner if Firebase fails quietly
    ...
    const handleDelete = async (id: string) => {
        ...
        if (!confirm('Permanently remove this resolved report from your history?')) return;

        setDeletingId(id);
        try {
            await incidentService.deleteIncident(id, user.uid, profile.role || 'student');
        } catch (error: any) {
                ...
                <div className="space-y-6">
                    ...
                    {loading ? (
                        ...
                        <motion.div variants={cardVariant} className="text-center py-20 glass rounded-3xl border border-white/5">
                            ...
                            <p className="text-[10px] text-zinc-500 mt-2 font-bold font-heading">You can find and manage your resolved reports here.</p>
                        </motion.div>
                    ) : (
                        <motion.div
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="min-w-0">
                                                    <h4 className="font-black text-white capitalize leading-tight truncate font-heading text-base drop-shadow-sm">
                                                        {(report.category || 'Incident').replace('_', ' ')}
                                                    </h4>
                                                    <p className="text-[10px] font-black text-[#D4AF37] uppercase tracking-widest mt-0.5 font-heading">
                                                        #{report.id?.slice(-6).toUpperCase() || 'UNKNOWN'} • {formatDate(report.createdAt)}
                                                    </p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                {report.status === 'resolved' && (
                                                    <button
                                                        ...
                                                        className="p-2 rounded-xl bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500/20 transition-all active:scale-90"
                                                        title="Delete report"
                                                    >
                                            ...
                                            >
                                                <div className="mt-6 space-y-6 pt-6 border-t border-white/5">
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <DetailBox label="Location" value={typeof report.location === 'string' ? report.location : 'Campus Map'} icon={<MapPin size={12} />} />
                                                        <DetailBox label="Hostel" value={report.hostelId || 'N/A'} icon={<MapPin size={12} />} />
                                                    </div>
                                                    ...
                                                    <div className="bg-black/40 p-5 rounded-2xl border border-white/5 shadow-inner">
                                                        <h5 className="text-[10px] font-black text-[#D4AF37] uppercase tracking-widest mb-6 flex items-center gap-2 font-heading">
                                                            <Activity className="w-3.5 h-3.5" /> History Log
                                                        </h5>
                                                        <div className="space-y-6 relative ml-1">
                                                            <div className="absolute left-[7px] top-2 bottom-2 w-0.5 bg-white/5" />
                                                            {report.timeline?.map((item, idx) => (
                                                                <div key={idx} className="relative pl-6 text-left">
                                                                    <div className="absolute left-[-4px] top-1.5 w-2 h-2 rounded-full bg-[#D4AF37] border border-black shadow-[0_0_10px_rgba(212,175,55,0.4)]" />
                                                                    <p className="text-xs font-black text-white font-heading">{item.action}</p>
                                                                    {item.note && <p className="text-xs text-zinc-500 mt-1 font-bold italic">"{item.note}"</p>}
                                                                    <p className="text-[9px] font-black text-zinc-600 mt-2 uppercase">
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-full py-4 bg-white/5 text-[#D4AF37] font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl border border-white/10 hover:bg-white/10 transition-all font-heading"
                                                    >
                                                        Collapse Info
                                                    </button>
                                                </div>
