        ...
        <MobileWrapper>
            ...
            >
                ...
                <motion.div variants={cardVariant} className="glass-card p-6 rounded-[2rem] border border-white/50 shadow-2xl flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-black text-slate-800 tracking-tight">Campus Safety Monitoring</h2>
                        <p className="text-sm text-slate-500 font-medium opacity-80">{activeWalks.length} active students</p>
                    </div>
                ...
                <div className="space-y-6">
                    <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2">Registered Walks</h3>
                    ...
                    {activeWalks.length === 0 ? (
                        <motion.div variants={cardVariant} className="glass-card p-12 rounded-[2.5rem] text-center border border-white/50 shadow-xl">
                            ...
                            <p className="text-slate-400 font-bold">No students on safe walk</p>
                        </motion.div>
                    ) : (
                        <div className="space-y-4">
                            {activeWalks.map((walk) => (
                                <motion.div
                                    key={walk.id}
                                    layoutId={walk.id}
                                    onClick={() => navigate(`/warden/safe-walk/${walk.id}`)}
                                    className="glass-card p-6 rounded-[2.2rem] cursor-pointer transition-all border-2 shadow-xl hover:-translate-y-1 active:scale-95 border-white/50 hover:border-white/80"
                                >
                                    ...
                                    <div className="bg-white/30 backdrop-blur-md p-4 rounded-2xl border border-white/60 space-y-3 shadow-sm text-sm">
                                        ...
                                        <div className="flex justify-between items-center pt-3 border-t border-white/40 text-[10px] font-black text-slate-400 uppercase tracking-[0.1em]">
                                            ...
                                            <span className="bg-primary/5 text-primary px-3 py-1 rounded-lg border border-primary/10">{walk.expectedDuration}m limit</span>
                                        </div>
                                    </div>
                                </motion.div>
                            ))}
